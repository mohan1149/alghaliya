    ...
    $('#customer_id').on('change', () => {
        ...
        if (cid !== '1') {
            $.getJSON("/api/get/customer-subscription-details/" + cid, (response) => {
                ...
                let cust_address = "";
                cust_address += (response.custom_field4 != null) ? response.custom_field4 + ',' : "";
                cust_address += (response.city != null) ? response.city + ',' : "";
                cust_address += (response.state != null) ? response.state + ',' : "";
                cust_address += (response.country != null) ? response.country + ',' : "";
                cust_address += (response.landmark != null) ? response.landmark + ',' : "";

                $("#cust_address").val(cust_address);
                $(".cu_add_sh").show();

                if (response.customer_group_id == "26") {
		    ...
		    localStorage.setItem('edit_old_amount',$("#final_total_input").val());
                }
...
localStorage.setItem('trans_type', 'mem');
        ...
        let net_total_avail = (quota_left - today_brought) + parseFloat(localStorage.getItem('edit_old_amount'));
...
$(document).on('click', 'div.product_box', function(e) {
    var pdt_id = $(this).attr('data-variation_id');
    var modal = document.getElementById("myModal");
     modal.style.display = "block";
     $('#myModal').empty();
     var url = '/sells/pos/get-pdtservice-List/'+pdt_id;
    $.ajax({
        method: 'GET',
        url: url,
        data: {
