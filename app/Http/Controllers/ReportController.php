...
class ReportController extends Controller
{
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $location_id = $request->get('location_id', null);
            $parent_payment_query_part = empty($location_id) ? "AND transaction_payments.parent_id IS NULL" : "";
            $cust_gp = $request->get('customer_group_id', null);
            $cust_gp_filter = !empty($cust_gp) ? "AND c.customer_group_id=$cust_gp" :"";
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                ...
                ->where('transaction_payments.business_id', $business_id)
                ->where(function ($q) use ($business_id, $contact_filter1, $contact_filter2, $parent_payment_query_part,$cust_gp_filter) {
            ...
            $cust_gp = $request->get('customer_group_id');
            if($cust_gp != NULL){
              $query->where('c.customer_group_id','=',$cust_gp);
            }
        ...
        $cust_group = CustomerGroup::forDropdown($business_id,false,true);
        $payment_types = $this->transactionUtil->payment_types();

        return view('report.sell_payment_report')
            ->with(compact('business_locations', 'customers', 'payment_types','cust_group'));
    }
